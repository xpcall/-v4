...
function coroutine.tostate(co)
	assert(type(co)=="thread","thread expected, (got "..type(co)..")")
	return ffi.cast("lua_State*",loadstring("return "..tostring(co):match("0x%x+").."ULL")())
end

function coroutine.tovm(co)
	return newvm(coroutine.tostate(co))
end

function newvm(L)
	if type(L)=="thread" then
		L=ffi.cast("lua_State*",tonumber(tostring(L):match("0x%x+")))
	end
	...
	o=setmetatable({
		...
		push=function(...)
			...
			for i=1,select("#",...) do
				...
				elseif tpe=="function" then
					o.loadstring(string.dump(v))
				elseif tpe=="table" then
					o.loadstring("return "..serialize(v))
					if LC.pcall(0,1,0)~=0 then
						local err="serialization error: "..ffi.string(LC.tolstring(-1,nil))
						LC.settop(-2)
						error(err)
					end
				elseif ffi.istype("lua_CFunction",v) then
					LC.pushcfunction(v)
				elseif ffi.istype("void*",v) then
					LC.pushpointer(v)
				else
		...
		pop=function(n)
			...
			for i=n,1,-1 do
				...
				if tpe==0 then
					o[i]=nil
				elseif tpe==1 then
					o[i]=LC.toboolean(-1)==1
				elseif tpe==2 or tpe==7 then
					o[i]=LC.touserdata(-1)
				elseif tpe==3 then
					o[i]=LC.checknumber(-1)
				elseif tpe==4 then
					local len=LC.objlen(-1)
					o[i]=ffi.string(LC.tolstring(-1,nil),len)
				elseif tpe==5 then
					o[i]={}
				elseif tpe==6 then
					local ou=""
		...
		__call=function(s,...)
			...
			return o.dostring(table.unpack(p),...)
		end
	})
	return o
end
...
function newthread(func,...)
	...
	--pthread.mutex_init(mutex,nil)
	local r=pthread.create(thread,nil,nptr,nil)
	if r~=0 then
	...
	return {
		ptr=thread,
		join=function()
			local r=pthread.join(thread[0],nil)
			if r~=0 then
				error("error "..r.." joining thread",2)
			end
		end,
		kill=function()
			pthread.kill(thread[0],9)
			--nptr:free()
		end
	}
end

function dispatchthreads(threads,func,args)
	local targs={}
	for l1=1,threads do
		targs[l1]={}
	end
	for l1=1,#args do
		table.insert(targs[((l1-1)%threads)+1],args[l1])
	end
	local o={}
	for l1=1,threads do
		o[l1]=newthread(function(f,a)
			a=loadstring("return "..a)()
			for l1=1,#a do
				f(table.unpack(a[l1]))
			end
		end,func,serialize(targs[l1]))
	end
	for l1=1,#o do
		o[l1].join()
		o[l1].kill()
...
function cfunction(ptr)
	local co=coroutine.create(setfenv(function() return func end,{}))
	local L=ffi.cast("lua_State*",coroutine.tostate(co))
	C.lua_getfenv(L,-1)
	C.lua_pushstring(L,"func")
	C.lua_pushcclosure(L,ffi.cast("lua_CFunction",ptr),0)
	C.lua_settable(L,-3)
	C.lua_settop(L,-2)
	local sc,func=coroutine.resume(co)
	return func
end
